...
if tls12_check
  ...
  control 'tls12-1' do
    ...
    describe registry_key('HKLM\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.2\Client') do
      it { should exist }
      its('DisabledByDefault') { should eq 0 }
      its('Enabled') { should eq 1 }
    end
    describe registry_key('HKLM\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.2\Server') do
      it { should exist }
      its('DisabledByDefault') { should eq 0 }
      its('Enabled') { should eq 1 }
    end
  ...
  control 'sslv3-1' do
    ...
    describe registry_key('HKLM\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\SSL 2.0\Server') do
      it { should exist }
      its('DisabledByDefault') { should eq 0 }
      its('Enabled') { should eq 0 }
    end
